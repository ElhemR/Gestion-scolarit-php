...
if($lign_rs==1){ 
     $supp="DELETE FROM assure WHERE cin=$ens";
    mysqli_query($conn,$supp);
   $sup="DELETE FROM enseignant WHERE cin=$ens";
    mysqli_query($conn,$sup);
  echo '<script type="text/javascript">alert("Enseignant supprimé");</script>';
  
}
else {echo '<script type="text/javascript">alert("Le cin inséré ne coresspond à aucun enseignant");</script>';}
}
...
<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
 <h2> Gestion enseignants - Supprimer </h2>
